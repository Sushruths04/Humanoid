# Documentation Index

This is the organized entry point for the repository documentation.

Use this page instead of browsing the root for every note and runbook.

## Setup

- [Lightning SSH setup](./setup/LIGHTNING_SSH_SETUP.md)
- [Start Here](../START_HERE.md)
- [Lightning backup workflow](../LIGHTNING_BACKUP_WORKFLOW.md)
- [CPU to GPU machine switch](../CPU_TO_GPU_MACHINE_SWITCH.md)
- [Setup status](../SETUP-STATUS.md)

## Operations

- [Docker image reuse](../DOCKER_IMAGE_REUSE.md)
- [Remote workflow](../REMOTE_WORKFLOW.md)
- [Machine switch quick reference](../MACHINE_SWITCH_QUICK_REF.md)
- [Machine change runbook](../MACHINE_CHANGE_RUNBOOK.md)
- [Recovery guide](../RECOVERY_GUIDE.md)
- [Emergency transfer](../EMERGENCY-TRANSFER.md)

## Vision VLA

- [Vision VLA CNN runbook](../VISION_VLA_CNN_RUNBOOK.md)
- [Vision blocker report](../VISION_BLOCKER_REPORT.md)

## Results and Planning

- [Final results](../FINAL_RESULTS.md)
- [Future work](../FUTURE_WORK.md)
- [Future tasks](../FUTURE_TASKS_HUMANOID_ROBOT.md)

## Handoff

- [Agent handoff](../AGENT_HANDOFF.md)

## Where Things Live

- GitHub: code, scripts, and short markdown notes.
- Hugging Face: large checkpoints, logs, rollout videos, and dataset-sized artifacts.
- GHCR: the reusable Docker image.
- Lightning machine: temporary runtime state and disposable compute.

## Suggested Reading Order

1. [README](../README.md)
2. [Start Here](../START_HERE.md)
3. [Lightning SSH setup](./setup/LIGHTNING_SSH_SETUP.md)
4. [Docker image reuse](../DOCKER_IMAGE_REUSE.md)
5. [Vision VLA CNN runbook](../VISION_VLA_CNN_RUNBOOK.md)

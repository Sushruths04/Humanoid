...
if [ "$START_CP" -le 4 ]; then
    ...
    $PY -m programs.p4_cosmos_world_sim.cp44_rollout \
        --checkpoint "$LORA_OUT/lora_adapter.pt" \
        --model-dir "$COSMOS_CKPT" \
        --data "$DATA" \
        --num-episodes 4 \
        --denoise-steps 20 \
...
if [ "$START_CP" -le 5 ]; then
    echo ""
    echo "=== CP4.5: CEM planning demo (standalone, no Docker) ==="
    cd "$REPO_DIR"
    PYTHONPATH="/tmp/te_stub:/tmp/cosmos-predict2" \
    $PY -m programs.p4_cosmos_world_sim.cp45_plan \
        --checkpoint "$LORA_OUT/lora_adapter.pt" \
        --model-dir "$COSMOS_CKPT" \
        --data "$DATA" \
        --cem-samples 16 \
